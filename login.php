<?php
// pharmacy_management_system/login.php
session_start();
...
if (isset($_SESSION['user_id'])) {
    ...
    try {
        ...
        $userRole = $stmt->fetchColumn(); // Get just the role

        if ($userRole === 'Customer') {
            header('Location: customer_dashboard.php');
            exit();
        } elseif (in_array($userRole, array('Pharmacist', 'Admin'))) { // Use array() for PHP 6.9 compatibility
            // If staff user accidentally lands on customer login page while already logged in
            header('Location: staff_dashboard.php');
            exit();
        }
    } catch (PDOException $e) {
        // Log error and handle gracefully without showing sensitive info to user
        error_log("Error checking existing user role: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $password = isset($_POST['password']) ? $_POST['password'] : ''; // Keep password as is, trim before hashing/comparing for security
    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            ...
            // Verify password and check if the role is 'Customer'
            if ($user && password_verify($password, $user['password_hash']) && $user['role'] === 'Customer') {
                ...
                // For customers, also store customer_id if it exists
                $stmtCustomer = $pdo->prepare("SELECT customer_id FROM Customers WHERE user_id = ?");
                $stmtCustomer->execute(array($user['user_id'])); // Use array() for PHP 6.9 compatibility
                ...
                $feedbackMessage = 'Invalid username or password, or you are not a customer.';
                $feedbackStatus = 'error';
            }
        } catch (PDOException $e) {
...
// Display feedback message from session if redirected (e.g., from customer_register.php)
...
<head>
    ...
    <title>Customer Login - Woloma Pharmacy</title>
    ...
    <style>
        ...
        .register-link, .staff-login-link {
            margin-top: 20px;
            font-size: 0.95em;
            color: #555;
        }
        .register-link a, .staff-login-link a {
            color: #007bff;
            text-decoration: none;
            font-weight: bold;
            transition: color 0.2s ease, text-decoration 0.2s ease;
        }
        .register-link a:hover, .staff-login-link a:hover {
    ...
    <main>
        <div class="login-container">
            ...
            <div class="staff-login-link">
                <p>Are you staff? <a href="staff_login.php">Log in here.</a></p>
            </div>
        </div>
    </main>
    <footer>
